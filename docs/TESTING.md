...
# DNSPod (use --test-threads=1 to avoid rate limiting)
...
> **Note**: DNSPod has a rate limit of 20 requests per second. Use `--test-threads=1` to run tests sequentially and avoid `QuotaExceeded` errors.
