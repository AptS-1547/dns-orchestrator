...
use common::{get_test_record_data, TestContext, TestRecordType};
...
async fn test_cloudflare_list_domains() {
    ...
    println!("✓ list_domains 测试通过，共 {} 个域名", response.total_count);
}
...
async fn test_cloudflare_list_records() {
    ...
    let response = result.unwrap();
    println!("✓ list_records 测试通过，共 {} 条记录", response.total_count);
}
...
macro_rules! crud_test {
    ($test_name:ident, $record_type:expr, $type_name:expr) => {
        ...
        async fn $test_name() {
            ...
            assert!(create_result.is_ok(), "create_record 失败: {:?}", create_result);
            ...
            assert!(update_result.is_ok(), "update_record 失败: {:?}", update_result);
            ...
            assert!(delete_result.is_ok(), "delete_record 失败: {:?}", delete_result);
            println!("  ✓ 删除成功");
...
crud_test!(test_cloudflare_crud_aaaa_record, TestRecordType::Aaaa, "AAAA");
crud_test!(test_cloudflare_crud_cname_record, TestRecordType::Cname, "CNAME");
