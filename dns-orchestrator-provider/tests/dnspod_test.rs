...
crud_test!(test_dnspod_crud_a_record, TestRecordType::A, "A");
crud_test!(test_dnspod_crud_aaaa_record, TestRecordType::Aaaa, "AAAA");
crud_test!(
    test_dnspod_crud_cname_record,
    TestRecordType::Cname,
    "CNAME"
);
crud_test!(test_dnspod_crud_mx_record, TestRecordType::Mx, "MX");
crud_test!(test_dnspod_crud_txt_record, TestRecordType::Txt, "TXT");
crud_test!(test_dnspod_crud_caa_record, TestRecordType::Caa, "CAA");

// SRV 记录需要特殊格式的主机名（_service._protocol）
#[tokio::test]
#[ignore]
async fn test_dnspod_crud_srv_record() {
    skip_if_no_credentials!("DNSPOD_SECRET_ID", "DNSPOD_SECRET_KEY", "TEST_DOMAIN");

    let mut ctx = TestContext::dnspod().expect("创建测试上下文失败");
    let domain_id = ctx.find_domain_id().await.expect("找不到测试域名");

    // SRV 记录需要 _service._protocol 格式
    let record_name = common::generate_srv_test_record_name();
    let (create_data, update_data) = get_test_record_data(TestRecordType::Srv);

    println!("测试 SRV 记录: {}", record_name);
